...
const AirtimeForm = () => {
  ...
  const { loading, isError, buyAirtime, success, resetError } =
    useBuyAirtimeContext();
          ...
          <View>
            <View>
              <Text style={{ marginVertical: 10 }}>
                Service Provider: {selectedAirtime}
              </Text>
              <View style={{ flexDirection: "row", gap: 10 }}>
                {NetworkProviders.map((p, index) => (
                  <TouchableOpacity
                    key={index}
                    onPress={() => setSelectedAirtime(p.value)}
                  >
                    <View
                      style={
      ...
      <ApModal visible={modalVisible} onClose={handleCloseModal}>
        ...
        {success && (
          <View>
            ...
            <View style={{ justifyContent: "center", alignItems: "center" }}>
              <ApIcon size={45} name="check" type="FontAwesome" color="green" />
            </View>
            <ApButton
              label="Close"
              onPress={() => {
